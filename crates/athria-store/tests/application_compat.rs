...
fn execute(
    ...
    match operation {
        ...
        "get_training_summary" => app.get_training_summary(
            input["days"].as_i64().unwrap(),
            input["from"].as_str(),
            input["to"].as_str(),
        ),
        ...
        "get_calendar" => app
            .get_calendar(input["from"].as_str(), input["to"].as_str())
            .map(Value::Array),
        "update_planned_session" => {
            app.update_planned_session(input["id"].as_str().unwrap(), &input["value"])
        }
        "get_next_training_day" => app.get_next_training_day(input["onOrAfterDate"].as_str()),
        "preview_hevy" => app.preview_hevy(
            input["content"].as_str().unwrap().as_bytes(),
            input["fileName"].as_str().unwrap(),
        ),
        ...
        "commit_xunji" => {
            app.commit_xunji(&input["result"], input["attemptedAt"].as_str().unwrap())
        }
...
fn application_contract_fixture_matches() {
    ...
    assert_eq!(
        app.get_profile().unwrap()["preferredName"],
        json!("Compat Athlete")
    );
}
