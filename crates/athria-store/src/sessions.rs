...
fn timestamp_millis(session: &Value, key: &str) -> Result<i64> {
    let raw = text(session, key).ok_or_else(|| AthriaError::new(AthriaErrorCode::InvalidData, format!("session is missing required field `{key}`")))?;
    crate::tz::millis(raw)
}
...
fn normalized_token(value: Option<&str>) -> String {
    value
        ...
        .replace("biking", "ride")
        .chars()
        .filter(|character| character.is_ascii_lowercase() || character.is_ascii_digit() || ('\u{3400}'..='\u{9fff}').contains(character))
        .collect()
}

/// `normalizedExercise`: `normalizedToken` with equipment prefixes removed.
fn normalized_exercise(value: Option<&str>) -> String {
    normalized_token(value).replace("dumbbell", "").replace("barbell", "").replace("machine", "").replace("cable", "")
}
...
fn ratio_score(left: f64, right: f64, bands: &[(f64, f64)]) -> f64 {
    ...
    bands.iter().find(|(limit, _)| difference <= *limit).map(|(_, score)| *score).unwrap_or(0.0)
}
...
fn endurance_number(session: &Value, key: &str) -> f64 {
    session.get("endurance").and_then(|endurance| endurance.get(key)).and_then(Value::as_f64).unwrap_or(0.0)
}

fn inferred_domains(session: &Value) -> BTreeSet<String> {
    let declared: Vec<String> = array(session, "domains").iter().filter_map(Value::as_str).map(str::to_string).collect();
...
fn duplicate_score(left: &Value, right: &Value) -> Result<Option<f64>> {
    ...
    let shorter = (left_end - left_start).max(0).min((right_end - right_start).max(0));
    let overlap_ratio = if shorter > 0 { overlap as f64 / shorter as f64 } else { 0.0 };
    let date_only = text_or(left, "timePrecision", "exact") == "date_only" || text_or(right, "timePrecision", "exact") == "date_only";
    if date_only {
        let timezone = text(left, "timezone").or_else(|| text(right, "timezone")).unwrap_or("UTC");
        if crate::tz::local_date(text_or(left, "startAt", ""), timezone)? != crate::tz::local_date(text_or(right, "startAt", ""), timezone)? {
            return Ok(None);
        }
    ...
    if left_modality != "unknown" && right_modality != "unknown" && left_modality != right_modality && left_modality != "mixed" && right_modality != "mixed" {
        return Ok(None);
    }
    let left_sport = normalized_token(text(left, "sport"));
    let right_sport = normalized_token(text(right, "sport"));
    if left_modality == "endurance" && right_modality == "endurance" && !left_sport.is_empty() && !right_sport.is_empty() && left_sport != right_sport {
        return Ok(None);
    }
    ...
    let time_score = if date_only { 45.0 } else { start_score.max((overlap_ratio * 45.0).round()) };
    ...
    let duration_score = ratio_score(number(left, "durationMinutes"), number(right, "durationMinutes"), &[(0.05, 15.0), (0.1, 12.0), (0.2, 8.0), (0.35, 4.0)]);
    ...
    if !left_sets.is_empty() && !right_sets.is_empty() {
        content_score = (jaccard(&exercise_tokens(left_sets), &exercise_tokens(right_sets)) * 20.0).round();
    } else if left_modality == "endurance" && right_modality == "endurance" {
        if !left_sport.is_empty() && left_sport == right_sport {
            content_score += 10.0;
        }
        content_score += ratio_score(endurance_number(left, "distanceMeters"), endurance_number(right, "distanceMeters"), &[(0.05, 10.0), (0.1, 8.0), (0.2, 4.0)]);
    } else {
    ...
    Ok(Some(time_score + modality_score + duration_score + content_score))
}
...
fn information_score(session: &Value) -> i64 {
    let endurance_fields = session
        ...
        .map(|fields| fields.iter().filter(|(key, value)| key.as_str() != "heartRateZoneSeconds" && !value.is_null()).count())
        .unwrap_or(0);
    let mut score = 0i64;
...
fn representative_session(cluster: &[Observation]) -> &Value {
    cluster
        .iter()
        .min_by(|left, right| information_score(&right.session).cmp(&information_score(&left.session)).then_with(|| js_locale_compare(&left.id, &right.id)))
...
fn cluster_observations(observations: &[Observation]) -> Result<Vec<Vec<Observation>>> {
    ...
    for observation in observations {
        ...
        for candidate in observations {
            if candidate.id == observation.id || text_or(&candidate.session, "source", "") == text_or(&observation.session, "source", "") {
                continue;
            }
            if let Some(score) = duplicate_score(&observation.session, &candidate.session)? {
                if score >= 75.0 {
                    by_source.entry(text_or(&candidate.session, "source", "").to_string()).or_default().push((candidate.id.clone(), score));
                }
            }
        }
        for candidates in by_source.values_mut() {
            candidates.sort_by(|left, right| right.1.partial_cmp(&left.1).unwrap_or(Ordering::Equal).then_with(|| js_locale_compare(&left.0, &right.0)));
            let Some(best) = candidates.first() else { continue };
            let best_score = best.1;
            if candidates.get(1).is_some_and(|second| best_score - second.1 < 10.0) {
                for candidate in candidates.iter().filter(|candidate| best_score - candidate.1 < 10.0) {
    ...
    sorted.sort_by(|left, right| {
        let left_key = format!("{}:{}", text_or(&left.session, "source", ""), text_or(&left.session, "externalId", ""));
        let right_key = format!("{}:{}", text_or(&right.session, "source", ""), text_or(&right.session, "externalId", ""));
        js_locale_compare(&left_key, &right_key)
    });
    let mut clusters: Vec<Vec<Observation>> = sorted.into_iter().map(|observation| vec![observation]).collect();
    loop {
        let mut candidates: Vec<(usize, usize, f64)> = Vec::new();
        for left in 0..clusters.len() {
            for right in left + 1..clusters.len() {
                let sources: HashSet<&str> = clusters[left].iter().map(|item| text_or(&item.session, "source", "")).collect();
                if clusters[right].iter().any(|item| sources.contains(text_or(&item.session, "source", ""))) {
                    continue;
                }
                ...
                for a in &clusters[left] {
                    for b in &clusters[right] {
                        if ambiguous.contains(&format!("{}|{}", a.id, b.id)) || ambiguous.contains(&format!("{}|{}", b.id, a.id)) {
                            compatible = false;
                            break;
                        }
        ...
        candidates.sort_by(|left, right| right.2.partial_cmp(&left.2).unwrap_or(Ordering::Equal).then_with(|| js_locale_compare(&clusters[left.0][0].id, &clusters[right.0][0].id)));
        let Some(best) = candidates.first().copied() else { break };
...
fn planned_content_score(planned: &Value, actual: &Value) -> Result<f64> {
    ...
    for component in array(planned, "components") {
        let Some(prescription) = component.get("prescription") else { continue };
        if text_or(prescription, "kind", "") != "strength" {
            continue;
        }
        for exercise in array(prescription, "exercises") {
            if let Some(name) = text(exercise, "canonicalKey").or_else(|| text(exercise, "displayName")) {
    ...
    Ok(if !name.is_empty() && name == normalized_token(text(actual, "name")) { 5.0 } else { 0.0 })
}
...
fn plan_match_score(planned: &Value, actual: &Value, timezone: &str) -> Result<Option<f64>> {
    ...
    if crate::tz::local_date(text_or(actual, "startAt", ""), actual_timezone)? != text_or(planned, "scheduledDate", "") {
        return Ok(None);
    }
    let planned_domains: BTreeSet<String> = array(planned, "components")
        .iter()
        .filter_map(|component| component.get("domain").and_then(|domain| domain.get("value")).and_then(Value::as_str))
        .map(str::to_string)
        .collect();
    ...
    let domain_score = if planned_domains.len() == 1 && actual_domains.len() == 1 { 30.0 } else { 20.0 };
    let duration_score = ratio_score(number(planned, "durationMinutes"), number(actual, "durationMinutes"), &[(0.1, 15.0), (0.2, 12.0), (0.35, 8.0), (0.5, 4.0)]);
    Ok(Some(35.0 + domain_score + duration_score + planned_content_score(planned, actual)?))
}
...
impl SqliteStore {
    ...
    pub(crate) fn match_details(&self, owner_id: &str) -> Result<HashMap<String, Value>> {
        ...
        for row in rows {
            ...
            matches.insert(training_session_id, json!({ "plannedSessionId": planned_session_id, "method": method }));
        }
        Ok(matches)
    }
    ...
    pub(crate) fn source_summaries(&self, owner_id: &str) -> Result<HashMap<String, Vec<Value>>> {
        ...
        for row in rows {
            ...
            summaries.entry(training_session_id).or_default().push(json!({ "source": source, "externalId": external_id }));
        }
        Ok(summaries)
    }
    ...
    fn with_derived_match(
        ...
        let Value::Object(mut fields) = session.clone() else {
            return Err(AthriaError::new(AthriaErrorCode::InvalidData, "stored training session is not a JSON object"));
        };
        if let Some(domain) = overrides.get(&id) {
            fields.insert("domains".to_string(), json!([domain]));
            let missing: Vec<Value> = array(session, "missingFields").iter().filter(|item| item.as_str() != Some("domains")).cloned().collect();
            fields.insert("missingFields".to_string(), Value::Array(missing));
        }
        fields.insert("plannedSessionId".to_string(), plan_match.and_then(|value| value.get("plannedSessionId")).cloned().unwrap_or(Value::Null));
        fields.insert("planMatch".to_string(), plan_match.cloned().unwrap_or(Value::Null));
        let summary = sources
            .get(&id)
            .cloned()
        ...
        fields.insert("isPlanMatchExcluded".to_string(), Value::Bool(excluded.contains(&id)));
    ...
    pub fn list_sessions(&self, owner_id: &str, since: Option<&str>) -> Result<Vec<Value>> {
        let mut statement = self
            ...
            .map_err(database_error)?;
        let rows = statement.query_map(params![owner_id, since], |row| row.get::<_, String>(0)).map_err(database_error)?;
        ...
        stored
            .into_iter()
            .map(|session| Self::with_derived_match(&session, &matches, &sources, &excluded, &overrides))
            .collect()
    }

    /// `listSessionsBySource`: per-source observations for one importer.
    pub fn list_sessions_by_source(&self, source: &str, owner_id: &str, since: Option<&str>) -> Result<Vec<Value>> {
        let mut statement = self
            .sqlite()
            ...
            .map_err(database_error)?;
        let rows = statement
            .query_map(params![owner_id, source, since], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)))
            .map_err(database_error)?;
        let matches = self.match_details(owner_id)?;
        ...
        for row in rows {
            ...
            let plan_match = matches.get(&training_session_id).cloned().unwrap_or(Value::Null);
            let mut projected = session.clone();
            projected["plannedSessionId"] = plan_match.get("plannedSessionId").cloned().unwrap_or(Value::Null);
    ...
    pub fn replace_source_sessions(&self, input: &ReplaceSourceSessionsInput<'_>) -> Result<WriteCounts> {
        self.transaction(&mut || {
    ...
    pub(crate) fn insert_plan_match(&self, owner_id: &str, planned_session_id: &str, training_session_id: &str, method: &str, confidence: f64, evidence: &Value) -> Result<()> {
        ...
        if method == "manual"
            && existing_plan.as_ref().is_some_and(|(_, existing)| existing == training_session_id)
            && existing_workout.as_ref().is_some_and(|(_, existing)| existing == planned_session_id)
        {
            return Ok(());
        }
        if method == "manual" {
    ...
    pub fn rebuild_canonical_sessions(&self, owner_id: &str) -> Result<()> {
        let mut observations: Vec<Observation> = Vec::new();
        {
            ...
            for row in rows {
                ...
                observations.push(Observation { id, canonical_id, session: parse_json_column(&data)? });
            }
        }
        let mut existing: HashMap<String, Value> = HashMap::new();
        {
            let mut statement = self.sqlite().prepare("SELECT id, data FROM training_sessions WHERE owner_id = ?1").map_err(database_error)?;
            let rows = statement.query_map(params![owner_id], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))).map_err(database_error)?;
            for row in rows {
        ...
        let mut excluded = if has_exclusions { self.excluded_workouts(owner_id)? } else { HashSet::new() };
        let has_overrides = self.table_exists("training_session_type_overrides")?;
        let override_ids: HashSet<String> = if has_overrides { self.type_overrides(owner_id)?.into_keys().collect() } else { HashSet::new() };
        ...
        for cluster in cluster_observations(&observations)? {
            ...
            let mut id = member_ids.iter().find(|candidate| protected.contains(*candidate) && !used.contains(*candidate)).cloned();
            if id.is_none() {
                id = member_ids.iter().find(|candidate| existing.contains_key(*candidate) && !used.contains(*candidate)).cloned();
            }
            let id = match id {
                Some(value) => value,
                None => {
                    ...
                    incoming["source"] = json!(format!("incoming:{}", text_or(representative, "source", "")));
                    let mut reusables: Vec<(String, f64)> = Vec::new();
                    for (candidate, session) in &existing {
                        if used.contains(candidate) {
                            continue;
                        }
                        reusables.push((candidate.clone(), duplicate_score(&incoming, session)?.unwrap_or(-1.0)));
                    }
                    reusables.sort_by(|left, right| right.1.partial_cmp(&left.1).unwrap_or(Ordering::Equal).then_with(|| js_locale_compare(&left.0, &right.0)));
                    match reusables.first() {
                        Some((candidate, score)) if *score >= 75.0 => candidate.clone(),
                        _ => member_ids.iter().find(|candidate| !used.contains(*candidate)).cloned().unwrap_or_else(|| Uuid::new_v4().to_string()),
                    }
                }
            };
            ...
            if member_ids.iter().any(|candidate| excluded.contains(candidate)) {
                excluded.insert(id.clone());
            }
        }

        self.sqlite().execute("DELETE FROM training_sessions WHERE owner_id = ?1", params![owner_id]).map_err(database_error)?;
        ...
        for (source_row_id, canonical_id) in &assignments {
            self.sqlite()
                .execute("UPDATE training_session_sources SET training_session_id = ?1 WHERE id = ?2", params![canonical_id, source_row_id])
                .map_err(database_error)?;
        }
        self.sqlite()
            .execute(
        ...
        if has_exclusions {
            self.sqlite().execute("DELETE FROM workout_plan_exclusions WHERE owner_id = ?1", params![owner_id]).map_err(database_error)?;
            for training_session_id in excluded.iter().filter(|candidate| used.contains(*candidate)) {
                self.sqlite()
                    .execute(
    ...
    fn owner_scoped_ids(&self, table: &str, owner_id: &str) -> Result<HashSet<String>> {
        let mut statement = self
            .sqlite()
            .prepare(&format!("SELECT training_session_id FROM {table} WHERE owner_id = ?1"))
            .map_err(database_error)?;
        let rows = statement.query_map(params![owner_id], |row| row.get::<_, String>(0)).map_err(database_error)?;
    ...
    pub fn reconcile_plan_matches(&self, owner_id: &str) -> Result<()> {
        let Some(plan) = self.get_current_plan(owner_id)? else { return Ok(()) };
        let planned: Vec<&Value> = plan
            .get("mesocycle")
            .and_then(|mesocycle| mesocycle.get("weeks"))
            .and_then(Value::as_array)
            .map(|weeks| weeks.iter().flat_map(|week| array(week, "sessions").iter()).filter(|session| text_or(session, "status", "planned") != "skipped").collect())
            .unwrap_or_default();
        let planned_ids: HashSet<&str> = planned.iter().map(|session| text_or(session, "id", "")).collect();

        self.sqlite().execute("DELETE FROM plan_workout_matches WHERE owner_id = ?1 AND method = 'auto'", params![owner_id]).map_err(database_error)?;
        ...
        let occupied_plans: HashSet<&str> = manual.iter().map(|(planned_session_id, _)| planned_session_id.as_str()).collect();
        let occupied_workouts: HashSet<&str> = manual.iter().map(|(_, training_session_id)| training_session_id.as_str()).collect();
        let excluded = self.excluded_workouts(owner_id)?;
        let workouts: Vec<Value> = self
            .list_sessions(owner_id, None)?
            .into_iter()
            .filter(|session| !occupied_workouts.contains(text_or(session, "id", "")) && !excluded.contains(text_or(session, "id", "")))
            .collect();
        ...
        candidates.sort_by(|left, right| {
            right
                ...
                .then_with(|| js_locale_compare(text_or(left.0, "id", ""), text_or(right.0, "id", "")))
                .then_with(|| js_locale_compare(text_or(left.1, "id", ""), text_or(right.1, "id", "")))
        });
        ...
        for (session, workout, score) in &candidates {
            ...
            let plan_alternative = candidates
                .iter()
                .find(|(other_session, other_workout, _)| text_or(other_session, "id", "") == session_id && text_or(other_workout, "id", "") != workout_id)
                .map(|(_, _, score)| *score)
                .unwrap_or(f64::NEG_INFINITY);
            let workout_alternative = candidates
                .iter()
                .find(|(other_session, other_workout, _)| text_or(other_workout, "id", "") == workout_id && text_or(other_session, "id", "") != session_id)
                .map(|(_, _, score)| *score)
                .unwrap_or(f64::NEG_INFINITY);
            if score - plan_alternative < 10.0 || score - workout_alternative < 10.0 || used_plans.contains(session_id) || used_workouts.contains(workout_id) {
                continue;
            }
    ...
    pub fn set_training_session_plan_match(&self, owner_id: &str, training_session_id: &str, planned_session_id: Option<&str>, expected_revision: i64) -> Result<Value> {
        self.transaction(&mut || {
    ...
    /// automatic matches.
    pub fn clear_training_session_plan_exclusion(&self, owner_id: &str, training_session_id: &str) -> Result<Value> {
        self.sqlite()
            .execute("DELETE FROM workout_plan_exclusions WHERE owner_id = ?1 AND training_session_id = ?2", params![owner_id, training_session_id])
            .map_err(database_error)?;
        self.reconcile_plan_matches(owner_id)?;
    ...
    pub fn update_manual_training_session(&self, owner_id: &str, training_session_id: &str, start_at: Option<&str>, duration_minutes: Option<i64>) -> Result<Value> {
        self.transaction(&mut || {
            let row = self
                .sqlite()
                .query_row(
    ...
    pub fn set_training_session_type_override(&self, owner_id: &str, training_session_id: &str, domain: &str) -> Result<Value> {
    ...
    pub fn delete_manual_training_session(&self, owner_id: &str, training_session_id: &str) -> Result<Option<Value>> {
        self.transaction(&mut || {
            let deleted = self
                .sqlite()
                .execute(
...
mod tests {
    ...
    fn identical_sessions_from_two_sources_merge_into_one_canonical_record() {
        let store = store();
        let manual = run("manual", "m1", "11111111-1111-4111-8111-111111111111", "2026-09-10T10:00:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run");
        let hevy = run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:05:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run");
        ...
        assert_eq!(sessions[0]["sources"], json!([{ "source": "hevy", "externalId": "h1" }, { "source": "manual", "externalId": "m1" }]));
        assert_eq!(sessions[0]["id"], json!("22222222-2222-4222-8222-222222222222"));

        let counts = store
            .upsert_sessions(&[run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:05:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run")])
            .unwrap();
    ...
    fn completed_sessions_auto_match_the_planned_session() {
        ...
        assert!(store.list_sessions(DEFAULT_OWNER_ID, None).unwrap().is_empty());

        store
            .upsert_sessions(&[run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:00:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run")])
            .unwrap();
        let sessions = store.list_sessions(DEFAULT_OWNER_ID, None).unwrap();
        ...
        let (confidence, evidence): (i64, String) = store
            .sqlite()
            .query_row("SELECT confidence, evidence FROM plan_workout_matches WHERE owner_id = ?1", params![DEFAULT_OWNER_ID], |row| {
    ...
    fn skipped_planned_sessions_never_auto_match() {
        ...
        store
            .upsert_sessions(&[run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:00:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run")])
            .unwrap();
        assert_eq!(store.list_sessions(DEFAULT_OWNER_ID, None).unwrap()[0]["plannedSessionId"], Value::Null);
    }
    ...
    fn manual_plan_links_win_over_auto_matching() {
        let store = store();
        store.save_current_plan(&plan_with_run(), 0).unwrap();
        let session = run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:00:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run");
        store.upsert_sessions(&[session]).unwrap();
        store.set_training_session_plan_match(DEFAULT_OWNER_ID, "22222222-2222-4222-8222-222222222222", None, 1).unwrap();
        store
            .set_training_session_plan_match(DEFAULT_OWNER_ID, "22222222-2222-4222-8222-222222222222", Some("s1"), 1)
            .unwrap();

        let sessions = store.list_sessions(DEFAULT_OWNER_ID, None).unwrap();
        assert_eq!(sessions[0]["planMatch"], json!({ "plannedSessionId": "s1", "method": "manual" }));

        let (method, confidence): (String, i64) = store
            .sqlite()
            .query_row("SELECT method, confidence FROM plan_workout_matches WHERE owner_id = ?1", params![DEFAULT_OWNER_ID], |row| {
                Ok((row.get(0)?, row.get(1)?))
            })
            .unwrap();
    ...
    fn merging_keeps_the_existing_canonical_id_and_the_better_source_payload() {
        ...
        let hevy = run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:03:00.000Z", "2026-09-10T11:02:00.000Z", 60, "Easy Run");
        store.upsert_sessions(&[hevy]).unwrap();

        let sessions = store.list_sessions(DEFAULT_OWNER_ID, None).unwrap();
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0]["id"], json!("11111111-1111-4111-8111-111111111111"));
    ...
    fn stored_sessions_keep_the_domains_the_importer_declared() {
        let store = store();
        let mut session = run("hevy", "h1", "22222222-2222-4222-8222-222222222222", "2026-09-10T10:00:00.000Z", "2026-09-10T11:00:00.000Z", 60, "Easy Run");
        session["domains"] = json!([]);
        session["modality"] = json!("mixed");
        store.upsert_sessions(&[session]).unwrap();
        let stored: String = store.sqlite().query_row("SELECT data FROM training_sessions", [], |row| row.get(0)).unwrap();
