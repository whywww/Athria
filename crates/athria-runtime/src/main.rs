...
fn run() -> Result<(), String> {
    ...
    match command {
        "doctor" => {
            println!(
                "{}",
                serde_json::to_string_pretty(&doctor(&path).map_err(|error| error.to_string())?)
        ...
        "serve" => {
            let token = std::env::var("ATHRIA_MCP_TOKEN")
                .map_err(|_| "ATHRIA_MCP_TOKEN is required for `athria serve`.".to_string())?;
            let address =
                std::env::var("ATHRIA_ADDRESS").unwrap_or_else(|_| "127.0.0.1:37373".to_string());
            if !(address.starts_with("127.0.0.1:") || address.starts_with("localhost:")) {
                return Err("athria serve only supports a loopback address.".to_string());
            }
...
mod tests {
    use super::*;
    #[test]
    fn parses_an_explicit_database() {
        let arguments = vec![
            "doctor".into(),
            "--database".into(),
            "custom.sqlite3".into(),
        ];
        assert_eq!(database_argument(&arguments), Ok(Some("custom.sqlite3")));
    }
    #[test]
    fn rejects_a_missing_database_value() {
        let arguments = vec!["doctor".into(), "--database".into()];
        assert!(database_argument(&arguments).is_err());
    }
}
