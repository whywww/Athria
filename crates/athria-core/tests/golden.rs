...
use athria_core::{
    DoubleProgressionInput, RpeAutoregulationInput, calculate_heart_rate_zones, calculate_training_metrics, estimate_one_rep_max, evaluate_double_progression,
    evaluate_rpe_autoregulation, expand_schedule, stable_hash, validate_plan,
};
use serde_json::Value;

const KINDS: [&str; 8] = ["double_progression", "heart_rate_zones", "one_rep_max", "rpe_autoregulation", "schedule", "stable_hash", "training_metrics", "validate_plan"];

fn run(kind: &str, input: &Value) -> Result<Value, String> {
    match kind {
        "schedule" => {
            let effective_start_date = input["effectiveStartDate"].as_str().expect("effectiveStartDate must be a string");
            let duration_weeks = input["durationWeeks"].as_i64().expect("durationWeeks must be an integer");
            serde_json::to_value(expand_schedule(effective_start_date, duration_weeks, &input["schedule"])).map_err(|error| error.to_string())
        }
        "one_rep_max" => {
            ...
            estimate_one_rep_max(load, reps, unit).map_err(|error| error.message().to_string()).and_then(serialize)
        }
        "heart_rate_zones" => {
            let max_heart_rate = input["maxHeartRate"].as_f64().expect("maxHeartRate must be a number");
            calculate_heart_rate_zones(max_heart_rate).map_err(|error| error.message().to_string()).and_then(serialize)
        }
        "training_metrics" => {
            let sessions = input["sessions"].as_array().cloned().expect("sessions must be an array");
            serialize(calculate_training_metrics(&sessions))
        }
        "double_progression" => {
            let parsed: DoubleProgressionInput = serde_json::from_value(input.clone()).map_err(|error| format!("double_progression input did not deserialize: {error}"))?;
            evaluate_double_progression(&parsed).map_err(|error| error.message().to_string()).and_then(serialize)
        }
        "rpe_autoregulation" => {
            let parsed: RpeAutoregulationInput = serde_json::from_value(input.clone()).map_err(|error| format!("rpe_autoregulation input did not deserialize: {error}"))?;
            evaluate_rpe_autoregulation(&parsed).map_err(|error| error.message().to_string()).and_then(serialize)
        }
...
fn compare(expected: &Value, actual: &Value, path: &str) -> Option<String> {
    match (expected, actual) {
        (Value::Number(left), Value::Number(right)) => {
            ...
            if (left - right).abs() <= tolerance { None } else { Some(format!("{path}: {left} vs {right}")) }
        }
        (Value::Array(left), Value::Array(right)) => {
            if left.len() != right.len() {
                return Some(format!("{path}: array length {} vs {}", left.len(), right.len()));
            }
            left.iter().zip(right).enumerate().find_map(|(index, (left, right))| compare(left, right, &format!("{path}[{index}]")))
        }
        (Value::Object(left), Value::Object(right)) => {
            ...
            right.keys().find(|key| !left.contains_key(*key)).map(|key| format!("{path}.{key}: extra on the Rust side"))
        }
...
fn golden_fixtures_match_the_typescript_core() {
    let directory = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("fixtures");
    let entries = fs::read_dir(&directory).unwrap_or_else(|error| panic!("cannot read {}: {error}; run `bun run scripts/core-golden.ts`", directory.display()));
    let mut paths: Vec<PathBuf> = entries
        .map(|entry| entry.expect("fixture directory entry").path())
        .filter(|path| path.extension().is_some_and(|extension| extension == "json"))
        .collect();
    paths.sort();
    assert!(!paths.is_empty(), "no fixtures in {}; run `bun run scripts/core-golden.ts`", directory.display());

    let mut failures: Vec<String> = Vec::new();
    let mut kinds_seen: BTreeSet<String> = BTreeSet::new();
    for path in &paths {
        let fixture: Value = serde_json::from_str(&fs::read_to_string(path).expect("fixture readable")).expect("fixture is JSON");
    ...
    let missing: Vec<&str> = KINDS.iter().copied().filter(|kind| !kinds_seen.contains(*kind)).collect();
    assert!(missing.is_empty(), "fixture kinds without coverage: {missing:?}");
    ...
    assert!(
        ...
        failures.len(),
        paths.len(),
        failures.iter().map(|failure| format!("  - {failure}")).collect::<Vec<_>>().join("\n")
    );
}
