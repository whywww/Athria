...
pub fn parse_iso_date(date: &str) -> (i64, u32, u32) {
    ...
    let year = parts
        .next()
        .expect("date must start with a year")
        .parse()
        .expect("year must be numeric");
    let month = parts
        .next()
        .expect("date must contain a month")
        .parse()
        .expect("month must be numeric");
    let day = parts
        .next()
        .expect("date must contain a day")
        .parse()
...
mod tests {
    ...
    fn epoch_day_round_trips_through_civil_dates() {
        for date in [
            ...
            assert_eq!(
                format_iso_date(epoch_day(date)),
                date,
                "{date} must round-trip"
            );
        }
