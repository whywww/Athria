...
pub const DOMAIN_IDS: [&str; 5] = ["strength", "endurance", "sport_skill", "mind_body", "recovery"];

pub const MOVEMENT_PATTERN_IDS: [&str; 33] = [
    "squat", "hinge", "lunge", "step", "bridge_hip_thrust", "horizontal_push", "vertical_push", "horizontal_pull", "vertical_pull", "shoulder_abduction",
    "shoulder_external_rotation", "elbow_flexion", "elbow_extension", "knee_extension", "knee_flexion", "hip_abduction", "hip_adduction", "calf_raise", "dorsiflexion", "carry",
...
pub const MUSCLE_GROUP_IDS: [&str; 47] = [
    ...
    "hamstrings", "thighs", "hips", "glutes", "calves", "lower_legs", "core", "spinal_erectors", "hip_flexors", "adductors",
    "abductors", "full_body", "other", "pectoralis_major_clavicular", "pectoralis_major_sternal", "latissimus_dorsi", "trapezius_upper", "trapezius_middle_lower", "rhomboids", "anterior_deltoid",
    "lateral_deltoid", "posterior_deltoid", "rotator_cuff", "biceps_brachii", "brachialis", "triceps_brachii", "forearm_flexors", "forearm_extensors", "gluteus_maximus", "gluteus_medius",
    "gluteus_minimus", "gastrocnemius", "soleus", "tibialis_anterior", "rectus_abdominis", "obliques", "transverse_abdominis",
];
...
fn muscle_parent(id: &str) -> Option<&'static str> {
    Some(match id {
        ...
        "latissimus_dorsi" | "trapezius_upper" | "trapezius_middle_lower" | "rhomboids" | "spinal_erectors" => "back",
        "anterior_deltoid" | "lateral_deltoid" | "posterior_deltoid" | "rotator_cuff" => "shoulders",
        "biceps_brachii" | "brachialis" | "triceps_brachii" | "forearm_flexors" | "forearm_extensors" => "arms",
...
pub const EQUIPMENT_CATEGORIES: &[EquipmentCategory] = &[
    EquipmentCategory {
        id: "strength_resistance", label: "Strength & Resistance", groups: &[
            EquipmentGroup { id: "free_weights", label: "Free Weights", items: &[("dumbbell", "Dumbbells"), ("barbell", "Barbell"), ("kettlebell", "Kettlebell")] },
            EquipmentGroup { id: "machines_cable", label: "Machines & Cable", items: &[("cable", "Cable Machine"), ("smith_machine", "Smith Machine"), ("machine", "Fixed Machines"), ("landmine", "Landmine")] },
            EquipmentGroup { id: "bodyweight_gymnastics", label: "Bodyweight & Gymnastics", items: &[("pull_up_bar", "Pull-Up Bar"), ("trx", "TRX"), ("bench", "Bench"), ("plyo_box", "Plyo Box")] },
            EquipmentGroup { id: "functional_gear", label: "Functional Gear", items: &[("resistance_band", "Resistance Bands"), ("medicine_ball", "Medicine Ball"), ("sandbag", "Sandbag"), ("sled", "Sled / Prowler")] },
        ],
    },
    EquipmentCategory {
        id: "cardio_endurance", label: "Cardio & Endurance", groups: &[
            EquipmentGroup {
                id: "indoor_cardio", label: "Indoor Cardio Machines",
                items: &[("treadmill", "Treadmill"), ("exercise_bike", "Exercise Bike"), ("rowing_machine", "Rowing Machine"), ("elliptical", "Elliptical"), ("stepper", "Stepper"), ("ski_erg", "SkiErg"), ("jump_rope", "Jump Rope"), ("battle_rope", "Battle Rope")],
            },
        ],
    },
    EquipmentCategory {
        id: "mobility_recovery", label: "Mobility, Pilates & Recovery", groups: &[
            EquipmentGroup { id: "mobility_tools", label: "Mobility Tools", items: &[("yoga_mat", "Yoga Mat"), ("foam_roller", "Foam Roller"), ("massage_ball", "Massage Ball")] },
            EquipmentGroup { id: "pilates_core", label: "Pilates & Core", items: &[("reformer", "Reformer"), ("pilates_ring", "Pilates Ring"), ("mini_stability_ball", "Mini Stability Ball")] },
        ],
    },
    EquipmentCategory {
        id: "ball_sport", label: "Ball & Sport-Specific Tools", groups: &[
            EquipmentGroup { id: "ball_sport_tools", label: "Ball & Sport-Specific Tools", items: &[("ball_machine", "Ball Machine")] },
        ],
    },
];
...
pub fn equipment_type_ids() -> Vec<&'static str> {
    EQUIPMENT_CATEGORIES.iter().flat_map(|category| category.groups.iter()).flat_map(|group| group.items.iter()).map(|(id, _)| *id).collect()
}
...
mod tests {
    ...
    fn default_profile_uses_the_schema_defaults() {
        ...
        assert_eq!(profile["equipment"].as_array().unwrap().len(), equipment_type_ids().len());
    ...
    fn muscle_taxonomy_exposes_parents_for_specific_muscles() {
        ...
        let latissimus = entries.iter().find(|entry| entry["id"] == json!("latissimus_dorsi")).unwrap();
        assert_eq!(latissimus["parentId"], json!("back"));
        assert_eq!(latissimus["label"], json!("Latissimus Dorsi"));
        let glutes = entries.iter().find(|entry| entry["id"] == json!("glutes")).unwrap();
