...
const SPORT_ROLES: [&str; 8] = ["preparation", "technical", "tactical", "small_sided_game", "match", "competition", "conditioning", "cool_down"];
...
const STRENGTH_VARIABLES: [&str; 10] = ["exercise_selection", "sets", "repetitions", "duration", "load", "rpe", "rir", "rest", "tempo", "alternatives"];
const ENDURANCE_VARIABLES: [&str; 12] =
    ["repetitions", "duration", "distance", "pace", "heart_rate_zone", "power", "cadence", "rpe", "talk_test", "terrain", "strides", "recovery_mode"];
const SPORT_VARIABLES: [&str; 6] = ["drill", "participants", "position", "duration", "intensity", "instructions"];
const RECOVERY_VARIABLES: [&str; 5] = ["body_region", "movement", "duration", "intensity", "instructions"];
const MIND_BODY_VARIABLES: [&str; 4] = ["technique", "duration", "intensity", "instructions"];

const DOMAINS: [&str; 5] = ["strength", "endurance", "sport_skill", "recovery", "mind_body"];
...
pub fn parse_session_template(value: &Value) -> Result<Value> {
    ...
    template.insert("id".into(), Value::String(required_text(value, "id", "template")?));
    template.insert("name".into(), Value::String(required_text(value, "name", "template")?));
    template.insert("intent".into(), Value::String(required_text(value, "intent", "template")?));
...
pub fn builtin_session_template(template: &Value, catalog_version: &str) -> Result<Value> {
    ...
    stored.insert("catalogVersion".into(), Value::String(catalog_version.to_owned()));
    Ok(Value::Object(stored))
}
...
pub fn parse_session_template_create(value: &Value) -> Result<(Value, Option<String>)> {
    ...
    Ok((template, value.get("clientRequestId").and_then(Value::as_str).map(str::to_owned)))
}
...
pub fn parse_session_template_update(value: &Value) -> Result<(Value, i64)> {
    ...
    let expected_revision = get(value, "expectedRevision").as_i64().ok_or_else(|| invalid_type("templateUpdate.expectedRevision", "an integer"))?;
...
fn parse_node(node: &Value, domain: &str, path: &str) -> Result<Value> {
    ...
    let role = node.get("role").and_then(Value::as_str).filter(|role| roles.contains(role)).ok_or_else(|| invalid(&format!("{path}.role"), "expected a node role"))?;
    let required_variables = parse_variables(node.get("variables"), variables, &format!("{path}.variables"))?;
    let optional_variables = match node.get("optionalVariables") {
        Some(value) => Some(parse_variables(Some(value), variables, &format!("{path}.optionalVariables"))?),
        None => None,
    };
    if let Some(optional) = &optional_variables {
        if optional.iter().any(|variable| required_variables.contains(variable)) {
            return Err(invalid(&format!("{path}.optionalVariables"), "required and optional variables must not overlap"));
        }
    }
    ...
    parsed.insert("variables".into(), Value::Array(required_variables.into_iter().map(Value::String).collect()));
    if let Some(optional_variables) = optional_variables {
        parsed.insert("optionalVariables".into(), Value::Array(optional_variables.into_iter().map(Value::String).collect()));
    }
    if domain == "strength" {
        let movement = parse_id_list(node.get("movementPatternIds"), &MOVEMENT_PATTERN_IDS, &format!("{path}.movementPatternIds"))?;
        let muscles = parse_id_list(node.get("targetMuscleIds"), &MUSCLE_GROUP_IDS, &format!("{path}.targetMuscleIds"))?;
        if movement.is_none() && muscles.is_none() {
            return Err(invalid(&format!("{path}.movementPatternIds"), "a strength node needs a movement pattern or target muscle"));
        }
        if let Some(movement) = movement {
            parsed.insert("movementPatternIds".into(), Value::Array(movement.into_iter().map(Value::String).collect()));
        }
        if let Some(muscles) = muscles {
            parsed.insert("targetMuscleIds".into(), Value::Array(muscles.into_iter().map(Value::String).collect()));
        }
...
fn parse_variables(value: Option<&Value>, allowed: &[&str], path: &str) -> Result<Vec<String>> {
    ...
    for entry in items {
        let candidate = entry.as_str().filter(|candidate| allowed.contains(candidate)).ok_or_else(|| invalid(path, "unknown template variable"))?;
...
fn parse_id_list(value: Option<&Value>, allowed: &[&str], path: &str) -> Result<Option<Vec<String>>> {
    ...
    for entry in items {
        let candidate = entry.as_str().filter(|candidate| allowed.contains(candidate)).ok_or_else(|| invalid(path, "unknown taxonomy id"))?;
        parsed.push(candidate.to_owned());
    }
    Ok(Some(parsed))
}
...
mod tests {
    ...
    fn parses_the_builtin_endurance_shape_in_schema_order() {
        ...
        assert_eq!(template["nodes"][0], json!({ "role": "warm_up", "variables": ["duration"] }));
    }
    ...
    fn strength_nodes_require_a_movement_pattern_or_target_muscle() {
        ...
        assert!(error.message().contains("movement pattern or target muscle"));
    }
    ...
    fn stored_templates_append_origin_and_revision() {
        ...
        assert_eq!(keys, ["id", "name", "intent", "domain", "nodes", "origin", "revision"]);
