...
pub fn normalize_intervals_activity(item: &Value, resource: &str) -> Result<Option<Value>> {
    ...
    let duration_value = object
        .get("moving_time")
        .or_else(|| object.get("elapsed_time"))
        .or_else(|| object.get("duration"));
    let mut seconds = finite(duration_value).unwrap_or(0.0);
    if object.contains_key("duration") && seconds > 0.0 && seconds < 1000.0 {
        seconds *= 60.0;
    }
    let minutes = if seconds > 0.0 {
        (seconds / 60.0).round().max(1.0) as i64
    } else {
        0
    };
    ...
    let external = object
        ...
        .unwrap_or_else(|| {
            format!("{:x}", Sha256::digest(item.to_string().as_bytes()))[..24].to_owned()
        });
    let metric = |name: &str| {
        object
            .get(name)
...
pub struct SyncDateWindow {
    pub days: i64,
    pub range_start: String,
    pub range_end: String,
}

pub fn sync_date_window(
    last_success_at: Option<&str>,
    requested_days: Option<i64>,
    today: &str,
) -> Result<SyncDateWindow> {
    let range_end = today
        .get(..10)
        .filter(|value| date::is_iso_date(value))
        .ok_or_else(|| invalid("today must be an ISO timestamp"))?;
    let days = match requested_days {
        Some(days) => days.clamp(1, 365),
        None => last_success_at
            .and_then(|value| value.get(..10))
            .filter(|value| date::is_iso_date(value))
            .map(|start| (date::day_difference(start, range_end) + 1).clamp(1, 365))
            .unwrap_or(90),
    };
    let end_day = date::epoch_day(range_end);
    Ok(SyncDateWindow {
        days,
        range_start: date::format_iso_date(end_day - days + 1),
        range_end: range_end.into(),
    })
}

/// Executes the three independent Intervals endpoints with retry isolation.
pub fn fetch_intervals<C: HttpClient>(
    client: &C,
    api_key: &str,
    athlete_id: &str,
    today: &str,
    activities_oldest: Option<&str>,
    wellness_oldest: Option<&str>,
) -> Value {
    let end = &today[..10];
    ...
    let paths = [
        (
            "activities",
            format!(
                "/athlete/{athlete_id}/activities?oldest={}&newest={end}",
                activities_oldest
                    .map(str::to_owned)
                    .unwrap_or_else(|| date::format_iso_date(end_day - 90))
            ),
        ),
        (
            "wellness",
            format!(
                "/athlete/{athlete_id}/wellness?oldest={}&newest={end}",
                wellness_oldest
                    .map(str::to_owned)
                    .unwrap_or_else(|| date::format_iso_date(end_day - 42))
            ),
        ),
        (
            "events",
            format!(
                "/athlete/{athlete_id}/events?oldest={}&newest={}",
                date::format_iso_date(end_day - 14),
                date::format_iso_date(end_day + 14)
            ),
        ),
    ];
    ...
    for (name, path) in paths {
        let request = HttpRequest {
            method: "GET",
            url: format!("https://intervals.icu/api/v1{path}"),
            headers: vec![
        ...
        for attempt in 0..3 {
            match client.send(&request) {
                Ok(response) if [401, 403].contains(&response.status) => {
                    result = json!("Intervals.icu rejected the API key");
                    break;
                }
                Ok(response)
                ...
                Ok(response) if !(200..300).contains(&response.status) => {
                    result = json!(format!("Intervals.icu returned HTTP {}", response.status));
                    break;
                }
                Ok(response) if response.body.is_array() => {
                    result = response.body;
                    break;
                }
                Ok(_) => {
                    result = json!("Intervals.icu response was not a list");
                    break;
                }
                Err(message) => {
                    result = json!(message);
