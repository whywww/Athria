...
fn auth_failure(status: u16, message: &str) -> bool {
    ...
    compact.contains("apikeymissing")
        || compact.contains("apikeyinvalid")
        || message.contains("仅VIP可用")
        || message.contains("仅vip可用")
}
...
pub fn fetch_xunji_training<C: HttpClient>(
    client: &C,
    api_key: &str,
    days: i64,
    today: &str,
) -> std::result::Result<Value, XunjiAuthenticationError> {
    let days = days.clamp(1, 365);
    let end = today
        .get(..10)
        .filter(|value| date::is_iso_date(value))
        .unwrap_or("1970-01-01");
    let end_day = date::epoch_day(end);
    let dates: Vec<String> = (0..days)
        .map(|offset| date::format_iso_date(end_day - days + offset + 1))
        .collect();
    ...
    for datestr in &dates {
        ...
        let request = HttpRequest {
            method: "POST",
            url: "https://trains.xunjiapp.cn/api_trains_for_llm_v2".into(),
            headers: vec![
        ...
        let mut last_message = "Xunji request failed".to_owned();
        for attempt in 0..3 {
            match client.send(&request) {
                Ok(response) => {
                    let message = safe_message(
                        &response.body,
                        format!("Xunji returned HTTP {}", response.status),
                    );
                    if auth_failure(response.status, &message) {
                        return Err(XunjiAuthenticationError(message));
                    }
                    if (response.status == 429
                        || message.to_ascii_lowercase().contains("too frequent")
                        || response.status >= 500)
                        && attempt < 2
                    {
                    ...
                    let trains = container.and_then(Value::as_array).or_else(|| {
                        container
                            .and_then(|value| value.get("trains"))
                            .and_then(Value::as_array)
                    });
                    if let Some(items) = trains {
                        records.extend(items.iter().filter(|item| item.is_object()).cloned());
                        successful_dates.push(json!(datestr));
                        completed = true;
                    } else {
                        last_message = "Xunji returned an invalid response".into();
                    }
                    break;
                }
                Err(message) => {
        ...
        if !completed {
            errors.push(json!({ "datestr": datestr, "code": "request_failed", "message": last_message.chars().take(500).collect::<String>() }));
        }
    }
    Ok(
        json!({ "rangeStart": dates.first(), "rangeEnd": dates.last(), "successfulDates": successful_dates, "errors": errors, "records": records }),
    )
}
...
fn number(value: Option<f64>) -> Value {
    value.map(athria_core::js_number).unwrap_or(Value::Null)
}
...
pub fn normalize_xunji_training(record: &Value) -> Result<Value> {
    let object = record
        .as_object()
        .ok_or_else(|| invalid("Xunji record must be an object"))?;
    let movements: Vec<&Map<String, Value>> = object
        .get("movements")
        .and_then(Value::as_array)
    ...
    let start_ms = finite(object.get("start"))
        ...
        .or_else(|| {
            object
                .get("start")
                .and_then(Value::as_str)
                .and_then(|value| tz::millis(value).ok())
        })
        .unwrap_or_else(|| {
            missing.push(json!("startAt"));
            object
                .get("datestr")
                .and_then(Value::as_str)
                .filter(|value| date::is_iso_date(value))
                .and_then(|value| tz::millis(&format!("{value}T00:00:00.000Z")).ok())
                .unwrap_or(0)
        });
    let mut end_ms = finite(object.get("end"))
        .map(|value| {
            if value < 10_000_000_000.0 {
                value * 1000.0
            } else {
        ...
        .map(|value| value as i64)
        .unwrap_or_else(|| {
            missing.push(json!("endAt"));
            start_ms
        });
    if end_ms < start_ms {
        end_ms = start_ms;
        missing.push(json!("duration"));
    }
    let mut sets = Vec::new();
    for movement in strength {
        for (index, raw) in movement
            .get("sets")
            .and_then(Value::as_array)
            .into_iter()
            .flatten()
            .enumerate()
        {
            let Some(set) = raw.as_object() else {
                continue;
            };
            if set.get("done") == Some(&Value::Bool(false)) {
                continue;
            }
    ...
    let metrics: Vec<&Map<String, Value>> = cardio
        .iter()
        .flat_map(|movement| {
            let mut values = Vec::new();
            if let Some(value) = movement.get("metrics").and_then(Value::as_object) {
                values.push(value);
            }
            if let Some(raw_sets) = movement.get("sets").and_then(Value::as_array) {
                values.extend(
                    raw_sets
                        .iter()
                        .filter_map(|set| set.get("metrics"))
        ...
        .collect();
    let metric = |names: &[&str]| {
        metrics
            .iter()
            .find_map(|item| names.iter().find_map(|name| finite(item.get(*name))))
    };
    let distance = metric(&["distanceMeters", "distance_m"])
        .or_else(|| metric(&["distance"]).map(|value| value * 1000.0));
    let external = object
        .get("localid")
        .map(|value| {
            value
        ...
        .unwrap_or_else(|| {
            hash(&format!(
                "{}|{}|{}",
                object.get("datestr").unwrap_or(&Value::Null),
                object.get("start").unwrap_or(&Value::Null),
                object.get("title").unwrap_or(&Value::Null)
            ))
        });
