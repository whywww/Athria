...
describe("SQLite repository v6", () => {
  it("creates the template-library schema and stores current values", () => {
    ...
    expect(repository.sqlite.query("SELECT version FROM athria_migrations WHERE version=6").get()).toEqual({ version: 6 });
  });
  ...
  it("migrates only the latest approved plan and removes active legacy history", () => {
    ...
    expect(repository.getCurrentPlan()).toMatchObject({ title: "Latest", effectiveStartDate: "2026-09-08", revision: 1, mesocycle: { schedule: { kind: "fixed_week", days: [{ dayOfWeek: 0, templateIds: ["full-body"] }] } } });
  ...
  it("reopens v6 without recreating legacy plan tables", () => {
