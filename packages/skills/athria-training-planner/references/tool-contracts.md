...
Effort notation follows the domain. Every Strength exercise carries a numeric `targetRpe` on the 1–10 scale, with optional `targetRir` as the reserve-based alternative; every Endurance step, including repeat work and recovery, carries a relative `heartRateZone` label such as `Zone 1–2`. Zone labels are relative to the athlete's own heart-rate baseline and require no stored maximum heart rate; the free-text `rpe` field on endurance steps is secondary context, never the primary notation. `validate_current_plan` reports `STRENGTH_EFFORT_RPE` and `ENDURANCE_EFFORT_ZONE` as advisories when the notation is missing.
...
`weeks[].sessions` is the only stored planned-session state. Calendar and next-day results derive completion from the workout match rather than writing `completed` into the Current Plan. Use `update_planned_session` only after explicit user direction: `complete` creates a date-only manual Training History record and match, `skip` marks that Session in the Current Plan, `restore` undoes a skip, and `move_occurrence` updates its date and week.

Normal Training History ↔ Current Plan matching always belongs to Athria's deterministic reconciliation algorithm. An Agent must not infer or choose `plannedSessionId`, including while calling `record_training_session`. Call `override_training_session_plan_match` only after the user explicitly identifies, corrects, replaces, or rejects a particular workout match. A non-null `plannedSessionId` applies that exact user-directed same-day override. A null `plannedSessionId` records the user's explicit intentionally-unplanned decision and excludes the workout from subsequent automatic matching. `allow_automatic_plan_match` is the only reversal of that exclusion: call it only when the user explicitly asks to return the workout to automatic matching. Manual-source edits and removal never delete synced observations.
