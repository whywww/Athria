...
`weeks[].sessions` is the only stored planned-session state. Calendar and next-day results derive completion from the explicit workout match rather than writing `completed` into the Current Plan. Use `update_planned_session` only after explicit user direction: `complete` creates a date-only manual Training History record and match, `skip` marks that Session in the Current Plan, `restore` undoes a skip, and `move_occurrence` updates its date and week. Use `set_training_session_plan_match` to re-link a same-day canonical workout or explicitly mark it unplanned; `allow_automatic_plan_match` clears that exclusion. Manual-source edits and removal never delete synced observations.
