...
Build reusable Plan Schema v5 session templates, then save the single Current Mesocycle that references them.
...
6. List the template library and create or update templates first. Construct the complete Current Mesocycle with an explicit `effectiveStartDate`, one supported schedule (`fixed_week`, `flexible_week`, or `interval`) that references template IDs, contiguous phases, and adjustment rules. Use fixed weekdays only when the user has a weekday-anchored routine; use flexible weekly targets for variable dates and interval cadence for spacing such as every other day. Do not invent loads, RPE, training days, equipment, or injury status.
