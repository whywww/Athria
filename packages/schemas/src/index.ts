...
export const trainingSessionSourceSummarySchema = z.object({ source: z.string().min(1), externalId: z.string().min(1) }).strict();
export const planWorkoutMatchSummarySchema = z.object({ plannedSessionId: z.string().min(1), method: z.enum(["auto", "manual"]) }).strict();
export const trainingSessionSchema = z.object({
  ...
  timePrecision: z.enum(["exact", "date_only"]).default("exact"), sources: z.array(trainingSessionSourceSummarySchema).default([]), planMatch: planWorkoutMatchSummarySchema.nullable().default(null), isPlanMatchExcluded: z.boolean().default(false),
...
export const trainingSessionWriteSchema = trainingSessionSchema.omit({ ownerId: true, source: true, status: true, sources: true, planMatch: true, isPlanMatchExcluded: true }).extend({ id: z.string().min(1).optional(), externalId: z.string().min(1).optional() }).strict();
...
const wellnessFields = {
  restingHeartRateBpm: wellnessFieldSchema(z.number().nonnegative()), hrvRmssdMs: wellnessFieldSchema(z.number().nonnegative()), hrvSdnnMs: wellnessFieldSchema(z.number().nonnegative()), sleepSeconds: wellnessFieldSchema(z.number().int().nonnegative()), sleepScore: wellnessFieldSchema(z.number().min(0).max(100)), sleepQuality: wellnessFieldSchema(z.number().int().nonnegative()), avgSleepingHeartRateBpm: wellnessFieldSchema(z.number().nonnegative()), weightKg: wellnessFieldSchema(z.number().positive()), bodyFatPercent: wellnessFieldSchema(z.number().min(0).max(100)), vo2maxMlKgMin: wellnessFieldSchema(z.number().nonnegative()),
  spo2Percent: wellnessFieldSchema(z.number().min(0).max(100)), stepsCount: wellnessFieldSchema(z.number().int().nonnegative()), respirationRpm: wellnessFieldSchema(z.number().nonnegative()),
  fatigue: wellnessFieldSchema(z.number().nonnegative()), soreness: wellnessFieldSchema(z.number().nonnegative()), stress: wellnessFieldSchema(z.number().nonnegative()), mood: wellnessFieldSchema(z.number().nonnegative()), motivation: wellnessFieldSchema(z.number().nonnegative()), readiness: wellnessFieldSchema(z.number().nonnegative()), injuryScore: wellnessFieldSchema(z.number().int().nonnegative()), notes: wellnessFieldSchema(z.string().max(2000)),
  eftpWatts: wellnessFieldSchema(z.number().nonnegative()), wPrimeJoules: wellnessFieldSchema(z.number().nonnegative()), pMaxWatts: wellnessFieldSchema(z.number().nonnegative()),
};
...
export const personalInformationWriteSchema = z.object({ preferredName: z.string().trim().min(1).max(100), gender: genderSchema.nullable(), heightCm: z.number().min(50).max(250).nullable(), birthDate: birthDateSchema.nullable(), weightKg: z.number().min(20).max(500).nullable().optional(), unitSystem: unitSystemSchema.optional(), expectedSnapshotHash: z.string().min(1) }).strict();
...
export const plannedSessionActionReasonSchema = z.object({ reasonCode: z.enum(["schedule", "recovery", "health", "travel", "equipment_weather", "preference", "other"]), note: z.string().trim().max(500).optional() }).strict();
export const plannedSessionActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("skip"), expectedRevision: z.number().int().min(0), reason: plannedSessionActionReasonSchema.optional() }).strict(),
  z.object({ action: z.literal("restore"), expectedRevision: z.number().int().min(0) }).strict(),
  z.object({ action: z.literal("move_occurrence"), expectedRevision: z.number().int().min(0), scheduledDate: dateSchema, reason: plannedSessionActionReasonSchema.optional() }).strict(),
]);
