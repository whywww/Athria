...
describe("template library and current plan", () => {
  ...
  it("saves only one current plan and protects referenced templates", () => {
    ...
    expect(() => app.deleteTemplate("full-body", 1)).toThrowError(/weekdays/i);
    expect(app.saveCurrentPlan({ ...plan(app, 1), title: "Replacement" })).toMatchObject({ plan: { revision: 2, title: "Replacement" } });
  });
  ...
  it("uses the current plan for next-day sessions and requires future update policy", () => {
    ...
    app.saveNextTrainingDaySessions({ clientRequestId: "schedule", scheduledDate: "2026-09-07", expectedRevision: 0, mode: "append", sessions: [{ id: "planned", templateId: "full-body" }] });
