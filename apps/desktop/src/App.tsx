...
function Connections() {
  ...
  return <section className="connections-page">
    <section className="connections-section"><h2>Connected ({sources.added.length})</h2><div className="connected-sources-grid">
      ...
      {!intervalsStatus.isPending && !xunjiStatus.isPending && !hevyStatus.isPending && sources.added.length === 0 && <EmptyState title={tr("No connections yet")} description="Choose one of the available connections below to get started."/>}
    </div></section>
...
export function Backup() {
  ...
  return <Card title={tr("Manage database")}>
    <p><T>{"All your data is stored in one portable database. Your API keys are protected by your database password."}</T></p>
...
export function ChangePasswordModal({ value, error, busy, onChange, onClose, onSubmit }: { value: { currentPassword: string; password: string; confirmation: string }; error: unknown; busy: boolean; onChange: (value: { currentPassword: string; password: string; confirmation: string }) => void; onClose: () => void; onSubmit: () => void; }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="connection-modal" role="dialog" aria-modal="true" aria-labelledby="change-password-title">
    ...
    <div className="modal-body">
      ...
      <ErrorBanner error={error}/>
      <div className="modal-actions"><button type="button" className="secondary" disabled={busy} onClick={onClose}><T>{"Cancel"}</T></button><button type="button" disabled={busy || value.currentPassword.length === 0 || value.password.length === 0 || mismatch} onClick={onSubmit}>{busy ? "Changing Password…" : "Change Password"}</button></div>
    </div>
  </section></div>;
}
...
function PersonalInformationCard() {
  ...
  if (query.isPending) return <Card title={tr("Personal Information")}><Loading/></Card>;
  if (query.isError || !query.data) return <Card title={tr("Personal Information")}><ErrorBanner error={query.error}/></Card>;
  ...
  return <Card title={tr("Personal Information")} className="personal-information-card" action={form ? <div className="actions"><button className="secondary compact" onClick={cancel}><T>{"Cancel"}</T></button><button className="compact" disabled={!valid || save.isPending} onClick={submit}>{save.isPending ? "Saving…" : "Save"}</button></div> : <button className="secondary compact" onClick={begin}><span aria-hidden="true">✎</span><T>{"Edit"}</T></button>}>
...
export function AddAgentModal({ onClose }: { onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="connection-modal agent-modal" role="dialog" aria-modal="true" aria-labelledby="agent-modal-title">
      <header><div><h2 id="agent-modal-title">{manual ? "Manual Setup" : "Add Agent"}</h2>{!manual && <p><T>{"Connect Athria to your AI agent on this computer."}</T></p>}</div><button type="button" className="modal-close" aria-label={manual ? "Back to Add Agent" : "Close dialog"} onClick={dismiss}><AppIcon name="close"/></button></header>
      <div className="modal-body">
...
export function AgentIntegrations() {
  ...
  return <Card title={tr("Connect to Your AI Agents")} className="agent-integrations-card" action={<button type="button" className="secondary compact agent-add" onClick={() => setAdding(true)}><AppIcon name="plus"/><T>{"Add Agent"}</T></button>}>
    <p><T>{"Connect the AI agents on this computer. Athria backs up your settings before each change."}</T></p>
...
export function Settings() {
  ...
  return <div className="settings-page"><Card title={t("Language")} className="language-settings"><label><span>{t("Language")}</span><select aria-label={t("Language")} value={language} onChange={(event) => setLanguage(event.target.value as "en" | "zh-CN")}><option value="en"><T>{"English"}</T></option><option value="zh-CN"><T>{"简体中文"}</T></option></select></label></Card><PersonalInformationCard/><Card title={t("Session Templates")} className="session-templates-settings" action={<button type="button" className="secondary compact" onClick={() => setTemplateLibrary(true)}>{t("View all templates")}</button>}><p>{t("Manage reusable workout patterns for your training sessions.")}</p></Card><Card title={t("System Status")} className="system-card" action={<ServiceStatus/>}><p>{t("Athria runs locally and keeps your training data on this device.")}</p></Card><AgentIntegrations/><Backup/></div>;
}
